...
# ── Terminal: echo off only (keep ICANON so Korean IME works) ─────────────────
_old_term = None
...
def echo_off():
    ...
    new = list(_old_term)
    new[3] &= ~termios.ECHO   # turn off echo, keep ICANON for IME
...
def draw_chat(log, rows, cols, blink):
    ...
    cur = "█" if blink else "▏"
    sys.stdout.write(mv(rows, 1) + "\033[2K" + G2 + " ❯ " + CY + cur + RESET +
                     DM + G1 + "  한국어/영어 입력 후 Enter" + RESET)
